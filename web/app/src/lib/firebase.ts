...
function resolveApiBaseUrl(): string {
  const baseUrl = process.env.NEXT_PUBLIC_API_BASE_URL || "https://api.omi.me";
...
function buildStoredSession(payload: Record<string, unknown>): StoredSession {
  ...
  if (!isSupabaseSessionPayload(accessPayload)) {
    throw new Error(
      "Authentication callback did not return a valid Omi session",
    );
  }
  const refreshToken =
  ...
  return {
    ...
    expiresAt:
      Date.now() +
      coerceNumber(payload.expires_in ?? sessionPayload.expires_in, 3600) *
        1000,
    user: {
      uid,
      email:
  ...
  return new Promise<User | null>((resolve, reject) => {
    ...
    const interval = window.setInterval(() => {
      ...
      if (Date.now() - popupClosedAt > 1500) {
        rejectWithError(
          new Error("Authentication window closed before completion"),
        );
      }
    }, 250);
