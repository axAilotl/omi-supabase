...
if (missing.length > 0) {
  const noopServiceWorker = `self.addEventListener('install', () => self.skipWaiting());
self.addEventListener('activate', (event) => {
  event.waitUntil(self.clients.claim());
});
`;
  fs.writeFileSync(outputPath, noopServiceWorker);
  console.warn('Firebase messaging config not set; generated a no-op service worker instead.');
  process.exit(0);
}
