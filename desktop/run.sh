...
Options (via environment variables):
  OMI_SKIP_BACKEND=1       Skip starting the local Rust backend
  OMI_SKIP_AUTH=1          Ignored in Supabase mode; kept for compatibility
  ...
  OMI_PYTHON_API_URL="..."  Python backend URL (default: http://127.0.0.1:3030)
...
Examples:
  ./run.sh                                   # Full local dev (desktop backend + app)
  OMI_SKIP_BACKEND=1 ./run.sh                # App only (desktop backend running elsewhere)
...
if [ "$1" = "--yolo" ]; then
    echo "ERROR: --yolo is intentionally disabled in the public self-host repo."
    echo "Use Backend-Rust/.env plus your local Supabase/backend services instead."
    exit 1
fi
...
if [ ! -f ".env" ] && [ "$1" != "--yolo" ]; then
    ...
    echo "  PORT=10201"
    echo "  BASE_API_URL=http://127.0.0.1:10201"
    echo "  OMI_PYTHON_API_URL=http://127.0.0.1:3030"
    echo "  SUPABASE_URL=http://127.0.0.1:54321"
    echo "  SUPABASE_AUTH_URL=http://127.0.0.1:54321/auth/v1"
    echo "  SUPABASE_ANON_KEY=<from supabase status>"
    echo ""
    echo "Or skip the backend entirely:"
    echo "  OMI_SKIP_BACKEND=1 ./run.sh"
    echo "  (set OMI_API_URL and OMI_AUTH_URL in .env.app to point to an already-running desktop backend)"
    echo "==========================="
    exit 1
fi
...
if [ "${OMI_SKIP_BACKEND:-0}" != "1" ] && [ ! -f "$CREDS_PATH" ]; then
    substep "google-credentials.json not found — legacy GCP-backed desktop features will be disabled"
fi
if [ -f "$CREDS_PATH" ]; then
    export GOOGLE_APPLICATION_CREDENTIALS="$CREDS_PATH"
fi

# FIREBASE_PROJECT_ID is only needed for remaining legacy desktop routes.
if [ -z "$FIREBASE_PROJECT_ID" ] && [ "${OMI_SKIP_BACKEND:-0}" != "1" ]; then
    substep "FIREBASE_PROJECT_ID not set — legacy Firestore-backed desktop routes will be disabled"
fi
...
substep "Legacy compatibility project: ${FIREBASE_PROJECT_ID:-disabled} | Backend port: $BACKEND_PORT | Auth port: $AUTH_PORT"
cd - > /dev/null
...
if [ -f "Desktop/Sources/GoogleService-Info-Dev.plist" ]; then
    substep "Copying GoogleService-Info-Dev.plist"
    ...
    substep "No GoogleService-Info.plist found — skipping"
fi
...
# Bootstrap OMI_AUTH_URL — check env var first, then backend .env, then local backend
...
if ! grep -q "^OMI_PYTHON_API_URL=" "$APP_BUNDLE/Contents/Resources/.env"; then
    ...
    if [ -z "$PYTHON_API_URL" ]; then
        PYTHON_API_URL="http://127.0.0.1:3030"
        substep "OMI_PYTHON_API_URL not set — defaulting to local backend: $PYTHON_API_URL"
    fi
