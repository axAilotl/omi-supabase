...
use crate::models::{
    ...
    AppSummary, AssistantSettingsData, Category, ChatSessionDB, Conversation, DailySummarySettings,
    ...
    pub async fn update_assistant_settings(
        ...
        if data.floating_bar.is_some() || current.floating_bar.is_some() {
            ...
            if let Some(v) = api_key {
                m.insert("elevenlabs_api_key".into(), json!({"stringValue": v}));
            }
            let voice_id = new.elevenlabs_voice_id.or(cur.elevenlabs_voice_id);
            if let Some(v) = voice_id {
                m.insert("elevenlabs_voice_id".into(), json!({"stringValue": v}));
            }
            if !m.is_empty() {
